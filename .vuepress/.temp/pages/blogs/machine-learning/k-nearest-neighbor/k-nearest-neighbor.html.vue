...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kNN-model-example-2d.png" alt="kNN-model-example-2d"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-building.png" alt="kd-tree-building"></p>
<p>上述的构建过程结合下图可以看出，构建一个k-d tree即是将一个二维平面逐步划分的过程。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-building-2d.png" alt="kd-tree-building-2d"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-building-3d.png" alt="kd-tree-building-3d"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-1.jpg" alt="kd-tree-create-rabbit-1"></p>
<p>首先先沿x坐标进行切分，我们选出x坐标的中位点，获取最根部节点的坐标</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-2.jpg" alt="kd-tree-create-rabbit-2"></p>
<p>并且按照该点的x坐标将空间进行切分，所有x坐标小于6.27的数据用于构建左枝，x坐标大于6.27的点用于构建右枝。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-3.jpg" alt="kd-tree-create-rabbit-3"></p>
<p>在下一步中r = 0+1 = 1mod2 = 1对应y轴，左右两边再按照y轴的排序进行切分，中位点记载于左右枝的节点。得到下面的树，左边的x是指这该层的节点都是沿x轴进行分割的。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-4.jpg" alt="kd-tree-create-rabbit-4"></p>
<p>空间的切分如下</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-5.jpg" alt="kd-tree-create-rabbit-5"></p>
<p>下一步中r = 1+1 = 2 mod 2 = 0，对应x轴，所以下面再按照x坐标进行排序和切分，有</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-6.jpg" alt="kd-tree-create-rabbit-6"></p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-7.jpg" alt="kd-tree-create-rabbit-7"></p>
<p>最后每一部分都只剩一个点，将他们记在最底部的节点中。因为不再有未被记录的点，所以不再进行切分。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-8.jpg" alt="kd-tree-create-rabbit-8"></p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-create-rabbit-9.jpg" alt="kd-tree-create-rabbit-9"></p>
<p>就此完成了 kd 树的构造。</p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-1.jpg" alt="kd-tree-search-rabbit-1"></p>
<p>先忽略在编程上的实现，想一想一个人如何主观地执行。嗯，他一定会把在五角附近的一些点中，分别计算每一个的距离，然后选最近的15个。这样可能只需要进行二三十次距离计算，而不是300次。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-2.jpg" alt="kd-tree-search-rabbit-2"></p>
<p>如图，只对紫圈里的点进行计算。</p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-3.jpg" alt="kd-tree-search-rabbit-3"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-4.jpg" alt="kd-tree-search-rabbit-4"></p>
<p>再沿横向从中间切成四份</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-5.jpg" alt="kd-tree-search-rabbit-5"></p>
<p>再沿着竖向平分八份</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-6.jpg" alt="kd-tree-search-rabbit-6"></p>
<p>最后再沿横向切一次。这次有些区域是完全空白的，我们就把它舍弃不要了，得到 14 份：</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-7.jpg" alt="kd-tree-search-rabbit-7"></p>
<p>我们再按照上下左右的关系把切开的图片做成一个二叉树，树的每一个节点是一幅图，它的两个枝是这幅图平分出来的子图。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-8.jpg" alt="kd-tree-search-rabbit-8"></p>
<p>可以看出这个树状结构包含了很多局部性的信息，因为它的每一个节点都是按照上下或者左右进行平分的，因此如果两个点在树中的距离较近，那么它们的实际距离就是比较近的。</p>
<p>用平面图表示切分结果为</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-8-1.jpg" alt="kd-tree-search-rabbit-8-1"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-9.jpg" alt="kd-tree-search-rabbit-9"></p>
<p>在找到了爱心之后，我们沿着相同的路径向上攀爬。只爬了一节就发现了屁股上的两个纹身：</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-10.jpg" alt="kd-tree-search-rabbit-10"></p>
<p>这里看出，在8平分的情况下，爱心和月亮是在同一个区域的。在某种意义上来讲它们是“近”的，但是我们还不能确定它们是最近的，因此还要继续向上攀爬寻找。再继续向上爬两个节点，都没有出现爱心和月亮以外的纹身。在下面这个节点中</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-11.jpg" alt="kd-tree-search-rabbit-11"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-12.jpg" alt="kd-tree-search-rabbit-12"></p>
<p>二叉树分割上也相应地多出这两个纹身。我们想找到离爱心最近的纹身，所以依旧向下搜寻先找到爱心。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-13.jpg" alt="kd-tree-search-rabbit-13"></p>
<p>我们找来一张纸，记下在已访问节点中距离爱心最近的纹身和所对应的距离。现在这张纸还是空的。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-14.jpg" alt="kd-tree-search-rabbit-14"></p>
<p>向上爬了一节，发现那一节的另一个枝里有月亮，于是跑下去查看月亮的坐标，计算爱心和月亮的距离，并在纸上记录 (图形=月亮，距离=d1)。</p>
<p>再回到蓝圈的节点向上爬，继续向上爬。我们发现，d1（红线）大于爱心和分割线的距离（蓝线）。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-15.jpg" alt="kd-tree-search-rabbit-15"></p>
<p>也就是说分割线的另一边可能有更近的点，所以从另一个分枝开始向下搜，找到…</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-16.jpg" alt="kd-tree-search-rabbit-16"></p>
<p>在另一个分枝中我们追溯到圆圈，并计算它与爱心的距离 d2，发现 d2&gt;d1，比月亮远，所以丢弃不要。</p>
<p>再向上爬一个节，我们发现 d1（红线）大于爱心和切分线之间的距离（蓝线）</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-17.jpg" alt="kd-tree-search-rabbit-17"></p>
<p>因此，切分线的另一端可能有更近的纹身，因此我们从另一个树枝向下搜索…</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-18.jpg" alt="kd-tree-search-rabbit-18"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-19.jpg" alt="kd-tree-search-rabbit-19"></p>
<p>然后再向向上攀登一节，这次我们发现已经爬到了树的最顶端，则完成了搜索，纸上记载的 (叶子，d3)就是最近的纹身和对应的距离。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-20.jpg" alt="kd-tree-search-rabbit-20"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-21.jpg" alt="kd-tree-search-rabbit-21"></p>
<p>首先执行(一)，我们按照切分找到最底部节点。首先，我们在顶部开始</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-22.jpg" alt="kd-tree-search-rabbit-22"></p>
<p>和这个节点的x轴比较一下，</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-23.jpg" alt="kd-tree-search-rabbit-23"></p>
<p>p的x轴更小。因此我们向左枝进行搜索：</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-24.jpg" alt="kd-tree-search-rabbit-24"></p>
<p>这次对比y轴，</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-25.jpg" alt="kd-tree-search-rabbit-25"></p>
<p>p的y值更小，因此向左枝进行搜索：</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-26.jpg" alt="kd-tree-search-rabbit-26"></p>
<p>这个节点只有一个子枝，就不需要对比了。由此找到了最底部的节点(−4.6,−10.55)。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-27.jpg" alt="kd-tree-search-rabbit-27"></p>
<p>在二维图上是</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-28.jpg" alt="kd-tree-search-rabbit-28"></p>
<p>此时我们执行(二)。将当前结点标记为访问过，并记录下L=[(−4.6,−10.55)]。啊，访问过的节点就在二叉树上显示为被划掉的好了。</p>
<p>然后执行(三)，嗯，不是最顶端节点。好，执行(a)，我爬。上面的是(−6.88,−5.4)。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-29.jpg" alt="kd-tree-search-rabbit-29"></p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-30.jpg" alt="kd-tree-search-rabbit-30"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-31.jpg" alt="kd-tree-search-rabbit-31"></p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-32.jpg" alt="kd-tree-search-rabbit-32"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-33.jpg" alt="kd-tree-search-rabbit-33"></p>
<p>因此，这说明在分割线的另一端可能有更近的点。于是我们在当前结点的另一个分枝从头执行(一)。好，我们在红线这里：</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-34.jpg" alt="kd-tree-search-rabbit-34"></p>
<p>要用p和这个节点比较x坐标：</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-35.jpg" alt="kd-tree-search-rabbit-35"></p>
<p>p的x坐标更大，因此探索右枝(1.75,12.26)，并且发现右枝已经是最底部节点，因此启动(二)。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-36.jpg" alt="kd-tree-search-rabbit-36"></p>
<p>经计算，(1.75,12.26) 与p的距离是17.48，要大于p与L的距离，因此我们不将其放入记录中。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-37.jpg" alt="kd-tree-search-rabbit-37"></p>
<p>然后(三)判断出不是顶端节点，呼出(a)，往上爬一个节点。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-38.jpg" alt="kd-tree-search-rabbit-38"></p>
<p>该节点未被访问过，所以标记为已访问。执行(1)。</p>
<p>(1)出来一算，这个节点与p的距离是4.91，要小于p与L的最大距离6.62。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-39.jpg" alt="kd-tree-search-rabbit-39"></p>
<p>因此，我们用这个新的节点替代L中离p最远的(−4.6,−10.55)。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-40.jpg" alt="kd-tree-search-rabbit-40"></p>
<p>然后(2)又来了，我们比对p和当前节点的分割线的距离</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-41.jpg" alt="kd-tree-search-rabbit-41"></p>
<p>这个距离小于L与p的最小距离，因此我们要到当前节点的另一个枝执行 (一)。当然，那个枝只有一个点，直接到 (二)。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-42.jpg" alt="kd-tree-search-rabbit-42"></p>
<p>将此节点标记为已访问过，计算距离发现这个点离p比L更远，因此不进行替代。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-43.jpg" alt="kd-tree-search-rabbit-43"></p>
<p>(三)发现不是顶点，所以呼出(a)。我们向上爬，</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-44.jpg" alt="kd-tree-search-rabbit-44"></p>
<p>这个是已经访问过的了，所以再来（a），</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-45.jpg" alt="kd-tree-search-rabbit-45"></p>
<p>好，（a）再爬，</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-46.jpg" alt="kd-tree-search-rabbit-46"></p>
<p>啊！到顶点了。所以完了吗？当然不，还没轮到(三) 呢。现在是 (1) 的回合。</p>
<p>我们进行计算比对发现顶端节点与p的距离比L还要更远，因此不进行更新。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-47.jpg" alt="kd-tree-search-rabbit-47"></p>
<p>然后是(2)，计算p和分割线的距离发现也是更远。</p>
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/kd-tree-search-rabbit-48.jpg" alt="kd-tree-search-rabbit-48"></p>
<p>因此也不需要检查另一个分枝。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/scikit-learn-kd-tree.png" alt="scikit-learn-kd-tree"></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/knn-example-1-1.png" alt="knn-example-1-1"></p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/knn-example-1-2.png" alt="knn-example-1-2"></p>
<p>下面再进行概率预测，使用</p>
...
<p><img src="@source/blogs/machine-learning/k-nearest-neighbor/pic/knn-example-1-3.png" alt="knn-example-1-3"></p>
<p><strong>结语</strong></p>

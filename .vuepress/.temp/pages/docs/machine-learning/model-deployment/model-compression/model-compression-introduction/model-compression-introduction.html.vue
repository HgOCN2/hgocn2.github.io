<template><div><h1 id="模型压缩概述" tabindex="-1"><a class="header-anchor" href="#模型压缩概述" aria-hidden="true">#</a> 模型压缩概述</h1>
<ul>
<li><RouterLink to="/docs/machine-learning/model-deployment/model-compression/model-compression.html">返回上层目录</RouterLink></li>
</ul>
<p>模型压缩- 剪枝/量化/蒸馏/AutoML
原因：深度学习计算复杂度高，参数冗余。
解决方式：
（1）线性或非线性量化。
（2）结构或非结构剪枝。
...
（4）权重矩阵的低秩分解。（蒸馏）
目的：
优化精度、性能、存储……使得可以在一些场景和设备上进行相应模型的部署。</p>
<ol>
<li>
<p>剪枝
...
④ 直接随机减去通道。
⑥ 剪枝分为：结构式剪枝和非结构式剪枝。
（7）实现原理：压低权重，根据权重大的数据进行保留，没用的数值权重越来越小，逐渐消失。L1正则化，对数据的净输出做正则化。（净输出做归一化（norm）：使得一部分权重压低后，另一部分的权重会升高。）</p>
</li>
<li>
<p>量化
（1）基础理论：
...
⑤ 做QAT伪训练。（量化后可能精确度下降，根据性能要求，再做训练）</p>
</li>
<li>
<p>蒸馏
...
原文链接：https://blog.csdn.net/weixin_41809530/article/details/106377921</p>
</li>
</ol>
</div></template>

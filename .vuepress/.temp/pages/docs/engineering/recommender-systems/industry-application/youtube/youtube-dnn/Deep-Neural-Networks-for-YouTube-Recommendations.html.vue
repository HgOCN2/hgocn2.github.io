...
<ul>
<li><RouterLink to="/docs/engineering/recommender-systems/industry-application/youtube/youtube.html">返回上层目录</RouterLink></li>
...
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-dnn-paper.png" alt="youtube-dnn-paper"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-architecture.png" alt="youtube-architecture"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-recall.png" alt="youtube-recall"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-ranking.png" alt="youtube-ranking"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-recall-1.png" alt="youtube-recall-1"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-new-feature.png" alt="youtube-new-feature"></p>
...
<p>因为在第7问中，我们已经知道模型采用了expected watch time per impression作为优化目标，所以如果简单使用LR就无法引入正样本的watch time信息。因此采用weighted LR，将watch time作为正样本的weight，在线上serving中使用$e^{(Wx+b)}$做预测可以直接得到expected watch time的近似，完美。</p>
<p>这里我们详细陈述一下这个问题：</p>
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/youtube-ranking-1.png" alt="youtube-ranking-1"></p>
<p><em>YouTube深度学习推荐系统中Ranking Model的架构图</em></p>
<p>上图是YouTube推荐系统排序模型（Ranking Model）的架构图，我们不再重复讲解模型的细节，而是把关注的焦点放在最后的输出层：</p>
<p><strong>为什么Ranking Model采用了weighted logistic regression作为输出层？在模型serving过程中又为何没有采用sigmoid函数预测正样本的probability，而是使用$e^{(Wx+b)}$这一指数形式预测用户观看时长？</strong></p>
<p>对于传统的深度学习架构，输出层往往采用LR或者Softmax，在线上预测过程中，也是原封不动的照搬LR或者softmax的经典形式来计算点击率（广义地说，应该是正样本概率）。</p>
<p>而YouTube这一模型的神奇之处在于，输出层没有使用LR，而是采用了Weighted LR，模型serving没有采用sigmoid函数的形式，而是使用了$e^{(Wx+b)}$这一指数形式。按照原文说法，这样做预测的就是用户观看时长？？没有任何其他表情能像这位小哥一样表达我初读论文的感受。。What？？？</p>
<p><img src="@source/docs/engineering/recommender-systems/industry-application/youtube/youtube-dnn/pic/what.jpg" alt="what"></p>
<p>搞清楚这件事情并不是一件容易的事情，我们要从逻辑回归的<strong>本质意义</strong>上开始。</p>
<p>几乎所有算法工程师的第一堂课就是逻辑回归，也肯定知道逻辑回归的数学形式就是一个线性回归套sigmoid函数：
$$
h_{\theta}(x)=\dfrac{1}{1+e^{-\theta^Tx}}
$$
但为什么选择sigmoid函数？难道仅仅是sigmoid函数能把值域映射到0-1之间，符合概率的物理意义这么简单吗？</p>
<p>答案显然不会这么肤浅。</p>
<p>为解释这个问题，首先我们需要定义一个新的变量——<strong>Odds</strong>，中文可以叫<strong>发生比</strong>或者<strong>机会比</strong>。
$$
Odds=\frac{p}{1-p}
$$
假设一件事情发生的概率是p，那么<strong>Odds就是一件事情发生和不发生的比值</strong>。</p>
<p>如果对Odds取自然对数，再让ln(Odds)等于一个线性回归函数，那么就得到了下面的等式。
$$
logit(p)=ln(\frac{p}{1-p})=\theta_0+\theta_1x_1+\theta_2x_2
$$
其中ln(p/(1-p))就是大名鼎鼎的<strong>logit函数</strong>，logistics regression又名logit regression，上面的式子就是逻辑回归的由来。我们再做进一步运算，就可以转变成我们熟悉的逻辑回归的形式：
$$
\begin{aligned}
...
\end{aligned}
$$
到这里大家应该已经完全明白了LR的推导过程了。</p>
<p>那么再对$ln(Odds)=\theta^Tx$这个等式做一个小小的转换，两边取自然底数：
$$
\begin{aligned}
&amp;ln(Odds)=\theta^Tx\
...
\end{aligned}
$$
大家看到了吗，Youtube的Serving函数$e^{Wx+b}$计算的不是别的，正是Odds！</p>
<p>但我们还没有到达终点，因为Youtube要预测的明明是用户观看时长，怎么就成了Odds了？</p>
<p>这就要提到YouTube采用的独特的训练方式Weighted LR，这里的Weight，对于正样本i来说就是观看时长$T_i$，对于负样本来说，则指定了单位权重1。</p>
<p>Weighted LR的特点是，正样本权重w的加入会让正样本发生的几率变成原来的w倍，也就是说样本i的Odds变成了下面的式子：
$$
...
$$
而且由于YouTube采用了用户观看时长$T_i$作为权重，因此式子进一步等于$T_i\times p$，这里真相就大白了，由于p就是用户打开视频的概率，$T_i$是观看时长，因此$T_i\times p$就是用户观看某视频的期望时长！</p>
...
<ol>
<li>$e^{Wx+b}$这一指数形式计算的是Weighted LR的Odds；</li>
<li>Weighted LR使用用户观看时长作为权重，使得对应的Odds表示的就是用户观看时长的期望；</li>
<li>因此，Model Serving过程中$e^{Wx+b}$计算的正是观看时长的期望。</li>
</ol>

...
<ul>
<li><RouterLink to="/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/reinforcement-learning.html">返回上层目录</RouterLink></li>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/DRN-model-framework.png" alt="DRN-model-framework"></p>
...
<p>（3）MINOR UPDATE（<strong>次要更新</strong>）：在每个时间戳之后（即，在时间戳t1后），根据上一个用户u和新闻列表L的特征表征，以及反馈B，智能体G会通过比较利用网络Q和探索网络$\tilde{Q}$的推荐表现，对模型进行更新。如果$\tilde{Q}$的推荐结果好，则当前的网络就将朝着$\tilde{Q}$的方向去更新。否则，Q将保持不变。在每次推荐展示后都会进行次要（Minor）更新。</p>
<p>（4）MAJOR UPDATE（<strong>主要更新</strong>）：经历一段时间$T_R$后（即在时间戳t3后），智能体G将使用存在其内存中的用户反馈B和用户活跃度来更新Q网络。这里，我们使用经验回放技术来更新网络。特别地，智能体G会保留最近的历史点击和用户活跃度记录。每次进行更新时，智能体G将采样一批记录来更新模型。主要更新通常在某一特定的时间段后进行，比如每隔一个小时，在此期间，已经进行了数以千计的推荐展示，并且会收集他的反馈。</p>
<p>（5）重复步骤（1）-（4）。</p>
...
<p>考虑到之前提到的新闻推荐的动态特征和估计未来奖励的需要，我们使用深度Q网络（DQN）来对一个用户点击某个新闻的可能性进行建模。从强化学习的角度来看，用户是否点击某个新闻（以及之后要推荐的新闻）的可能性实质上就是智能体能够获得的奖励。因此，我们对整个奖励建模如式(1)所示：
$$
...
$$
y_{s,a,t}=r_{a,t+1}+\gamma Q(s_{a,t+1},\text{arg }\mathop{\text{max}}<em>{a'}Q(s</em>{a,t+1},a';W_t);W_t')\quad\quad\quad(2)
$$
其中，$r_{a,t+1}$为采取行动a的即时奖励（下标为t+1是因为奖励总是会比行动延迟1个时间步）。这里，Wt和W't是两个不同的DQN参数设置。在该式中，给定所选动作a，智能体G将推测下一个阶段$s_{a,t+1}$。基于此，给定动作{a'}的候选集，根据参数Wt，选择使得将来奖励最大化的动作a‘。在此之后，已知状态$s_{a,t+1}$，基于W't，计算出估计的将来奖励。每隔几次迭代，互换Wt和W‘t。已经证明该策略能消除Q估计值的过度优化。通过该处理，DQN可以同时根据即时奖励和将来将来进行决策。</p>
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/Q-Network.png" alt="Q-Network"></p>
...
<p>我们使用生存模型对用户返回和用户活跃度进行建模。生存分析已经用在了估计用户返回时间领域了。假设T是直到用户下次返回APP的时间长度，则风险函数（即事件发生的瞬时率）如式（3）所示：
$$
...
S(t)=S(t)+S_a
$$
。用户活跃度得分不会超过1。例如，如图5所示，该用户的活跃度在0时刻处从S0开始衰减。在时刻t1处，用户返回了APP，这导致用户的活跃度有一个Sa的上涨。然后，在时刻t1后，用户活跃度持续下跌。类似的事情发生在t2、t3、t4和t5。注意，在t4到t9之间，尽管该用户的请求频率已经很高了，但是用户活跃度的最大值依然被截断限制为1。</p>
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/user-activeness-estimation.png" alt="user-activeness-estimation"></p>
<p>参数
$$
S_0,\ S_a,\ \lambda_0,\ T_0
$$
根据数据集中的实际用户模式来确定。S0被设置为0.5，表示用户的随机初始状态（即可能是活跃的，也可能是不活跃的）。我们可以观察用户每两个连续请求之间的时间间隔的直方图，如图6所示。</p>
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/time-interval-between-two-consecutive-requests.png" alt="time-interval-between-two-consecutive-requests"></p>
<p>我们观察到，除了一天多次阅读新闻外，人们通常每天定期返回APP。所以我们将T0设置为24小时。根据式(4)和式(5)，衰减因子λ0设置为1.2e-5/s。此外，每次点击后，用户活跃度的增量Sa设置为0.32，以保证用户在每日一次请求后返回初始状态，即，
$$
...
$$
尽管这里我们使用生存模型来估计用户活跃度，但是其他的模型，如泊松点过程（Poisson point proces）也能提供相似的功能。</p>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/exploration-by-dueling-bandit-gradient-descent.png" alt="exploration-by-dueling-bandit-gradient-descent"></p>
<p>直观地，如图7所示，智能体G会使用当前网络Q产生推荐列表L，并使用探索网络$\tilde{Q}$来产生另一个推荐列表$\tilde{L}$。通过对当前网络Q的参数W增加小扰动ΔW（式7），来得到网络$\tilde{Q}$的参数$\tilde{W}$。
$$
...
$$
否则，智能体G将保持网络Q不变。通过这种探索，智能体会进行更有效的探索，同时推荐精度又不损失更多。</p>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/user-and-news-basic-illustration.png" alt="user-and-news-basic-illustration"></p>
...
D(r)=\frac{1}{log(1+r)}
$</p>
<p>其中，r表示物品在推荐列表中的排名，n是推荐列表的长度，f是排名函数或者算法，$y^f_r$是1或者0，表示是否点击会发生，D(r)是折扣。</p>
</li>
</ul>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/parameter-setting.png" alt="parameter-setting"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/offline-recommendation-accuracy.png" alt="offline-recommendation-accuracy"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/offline-cumulative-CTR-of-different-methods.png" alt="offline-cumulative-CTR-of-different-methods"></p>
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/online-recommendation-accuracy.png" alt="online-recommendation-accuracy"></p>
...
<p>（2）推荐多样性。最终，为了评估探索的效果，我们使用ILS来计算不同算法的推荐多样性，计算方法如式(13)所示：
$$
...
<p><img src="@source/docs/engineering/recommender-systems/advanced-knowledge/reinforcement-learning/drn/pic/diversity-of-user-clicked-news-in-the-online-experiment.png" alt="diversity-of-user-clicked-news-in-the-online-experiment"></p>

import{_ as i,r as e,o as c,c as l,b as n,e as a,w as u,d as s,a as p}from"./app-VitiyI7N.js";const r="/assets/ckpt-model-F6Oa3Dbl.png",d="/assets/output-node-LtLEL89C.png",k={},m=n("h1",{id:"tensorflow中ckpt转pb文件-模型持久化",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#tensorflow中ckpt转pb文件-模型持久化","aria-hidden":"true"},"#"),s(" TensorFlow中ckpt转pb文件（模型持久化）")],-1),v=p('<li><a href="#tensorflow%E6%A8%A1%E5%9E%8B%E6%96%87%E4%BB%B6%E8%A7%A3%E8%AF%BB">TensorFlow模型文件解读</a></li><li><a href="#%E8%8E%B7%E5%8F%96%E9%9C%80%E8%A6%81%E6%8C%81%E4%B9%85%E5%8C%96%E6%A8%A1%E5%9E%8B%E7%9A%84%E8%BE%93%E5%87%BA%E8%8A%82%E7%82%B9%E5%90%8D%E7%A7%B0">获取需要持久化模型的输出节点名称</a></li><li><a href="#ckpt%E6%96%87%E4%BB%B6%E7%94%9F%E6%88%90%E6%8C%81%E4%B9%85%E5%8C%96%E6%A8%A1%E5%9E%8Bpb%E6%96%87%E4%BB%B6">ckpt文件生成持久化模型pb文件</a></li><li><a href="#%E8%AF%BB%E5%8F%96%E7%94%9F%E6%88%90%E7%9A%84pb%E6%96%87%E4%BB%B6%E5%B9%B6%E6%89%93%E5%8D%B0%E8%8A%82%E7%82%B9%E5%90%8D%E7%A7%B0">读取生成的pb文件并打印节点名称</a></li><li><a href="#%E5%88%A9%E7%94%A8%E7%94%9F%E6%88%90%E7%9A%84pb%E6%96%87%E4%BB%B6%E5%AE%9E%E7%8E%B0%E6%8E%A8%E6%96%AD">利用生成的pb文件实现推断</a></li>',5),b=p('<h1 id="tensorflow模型文件解读" tabindex="-1"><a class="header-anchor" href="#tensorflow模型文件解读" aria-hidden="true">#</a> TensorFlow模型文件解读</h1><p>使用tensorflow训练好的模型会自动保存为四个文件，如下</p><p><img src="'+r+`" alt="ckpt-model"></p><ul><li><p>**checkpoint：**记录近几次训练好的模型结果（名称）。</p></li><li><p><strong>xxx.data-00000-of-00001:</strong> 模型的所有变量的值(weights, biases, placeholders,gradients, hyper-parameters etc)，也就是模型训练好的<strong>权重的值</strong>和其他值。</p></li><li><p>**xxx.index ：**模型的元数据，二进制或者其他格式，不可直接查看 。是一个不可变得字符串表，每一个键都是张量的名称，它的值是一个序列化的BundleEntryProto。 每个BundleEntryProto描述张量的元数据：“数据”文件中的哪个文件包含张量的内容，该文件的偏移量，校验和一些辅助数据等。</p></li><li><p><strong>xxx.meta：<strong>模型的meta数据 ，二进制或者其他格式，不可直接查看，保存了TensorFlow</strong>计算图的结构</strong>信息，通俗地讲就是<strong>神经网络的网络结构</strong>。该文件可以被<code>tf.train.import_meta_graph</code>加载到当前默认的图来使用。</p></li></ul><h1 id="获取需要持久化模型的输出节点名称" tabindex="-1"><a class="header-anchor" href="#获取需要持久化模型的输出节点名称" aria-hidden="true">#</a> 获取需要持久化模型的输出节点名称</h1><p>节点名称通常可以在正常的ckpt模型推断代码中找到。</p><p>在模型构建的代码的末尾打上断点，如下所示：</p><div class="language-python line-numbers-mode" data-ext="py"><pre class="language-python"><code><span class="token keyword">class</span> <span class="token class-name">DeepNet</span><span class="token punctuation">(</span><span class="token builtin">object</span><span class="token punctuation">)</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后调试，会发现你想要输出的tensor的别名，在本例中，想要输出的tensor是<code>mu</code>，如下图的黄框所示，其别名叫做<code>pi/mul</code>，注意不是<code>pi/mu:0</code>。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>name: <span class="token string">&quot;pi/mul&quot;</span>  <span class="token comment"># 名字叫这个</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="`+d+`" alt="output-node"></p><h1 id="ckpt文件生成持久化模型pb文件" tabindex="-1"><a class="header-anchor" href="#ckpt文件生成持久化模型pb文件" aria-hidden="true">#</a> ckpt文件生成持久化模型pb文件</h1><p>获取节点名称后通过ckpt文件持久化模型，生成pb文件</p><div class="language-python line-numbers-mode" data-ext="py"><pre class="language-python"><code><span class="token keyword">from</span> tensorflow<span class="token punctuation">.</span>python<span class="token punctuation">.</span>framework <span class="token keyword">import</span> graph_util
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>结果如下，一共有17个节点：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>state
...
pi/mul/x
pi/mul
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>之所以只剩下17个节点了，是因为这17个节点是前向传播的，其余的op算子都不属于前向传播推理，所以就被去掉了。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>op的打印顺序大致以 以下顺序排列：
前向传播op
占位符op
优化器op
评估指标（loss、acc）op
反向传播梯度op
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h1 id="利用生成的pb文件实现推断" tabindex="-1"><a class="header-anchor" href="#利用生成的pb文件实现推断" aria-hidden="true">#</a> 利用生成的pb文件实现推断</h1><div class="language-python line-numbers-mode" data-ext="py"><pre class="language-python"><code><span class="token keyword">from</span> tensorflow<span class="token punctuation">.</span>python<span class="token punctuation">.</span>framework <span class="token keyword">import</span> graph_util
<span class="token keyword">import</span> tensorflow <span class="token keyword">as</span> tf
<span class="token keyword">import</span> numpy <span class="token keyword">as</span> np
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最后的推断结果的为<code>[ 0.8672415 -0.55812573]</code>，和直接用模型预测的结果完全一致。</p><h1 id="参考资料" tabindex="-1"><a class="header-anchor" href="#参考资料" aria-hidden="true">#</a> 参考资料</h1>`,25),_={href:"http://www.kaotop.com/it/19844.html",target:"_blank",rel:"noopener noreferrer"},h=n("p",null,"开头的“tensorflow模型文件解读”参考了此博客。",-1),f={href:"https://zhuanlan.zhihu.com/p/147658249",target:"_blank",rel:"noopener noreferrer"},g=n("p",null,"本文主要参考了此博客。",-1);function w(y,E){const o=e("RouterLink"),t=e("ExternalLinkIcon");return c(),l("div",null,[m,n("ul",null,[n("li",null,[a(o,{to:"/docs/machine-learning/machine-learning-framework/tensorflow/tools/tools.html"},{default:u(()=>[s("返回上层目录")]),_:1})]),v]),b,n("ul",null,[n("li",null,[n("a",_,[s("tensorflow模型文件(ckpt)转pb文件的方法（不知道输出节点名）"),a(t)])])]),h,n("ul",null,[n("li",null,[n("a",f,[s("tensorflow中ckpt转pb文件（模型持久化）"),a(t)])])]),g])}const q=i(k,[["render",w],["__file","ckpt-to-pb.html.vue"]]);export{q as default};
